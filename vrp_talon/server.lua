  				...
  				local tsource = vRP.getUserSource({target_id})
					if tsource ~= nil then
						vRPclient.getNearestOwnedVehicle(tsource, {10}, function(ok1, vtype, name)
							if ok1 then
								vRP.request({tsource, "Doriti sa aratati talonul masinii ?", 30, function(ok2)
									if ok2 then
										vRPclient.notify(player, {"~g~Jucatorul ti-a oferit talonul"})
										  ...
										  vRP.getUserIdentity({target_id, function(identy)
											MySQL.query("vRP/talon_get_vehicle", {target_id = target_id, name = name}, function(rows, affected)
											  if #rows > 0 then
												...
												local content = "Nume proprietar: "..nume.."<br/>Codul masinii: "..name.."</br>Numar de inmatriculare: "..plate.."</br>Culoare Principala: "..color1.."</br>Culoare Secundara: "..color2..""
												vRPclient.setDiv(player, {"police_talon", ples_css, content})
												vRP.request({player, "Inchide", 1000, function(player, ok3)
												  vRPclient.removeDiv(player, {"police_talon"})
												end})
											  end
											end)
										  end})
										...
										vRPclient.notify(player, {"~r~Cerere refuzata"})
									end
								end})
							else
								vRPclient.notify(player, {"~r~Jucatorul ales nu detine nici o masina in apropiere"})
							end
						end)
  				else
  					vRPclient.notify(player, {"~r~Jucatorul nu este conectat"})
  				end
  			end})
  		else
  			vRPclient.notify(player, {"~r~Nu exista nici un jucator in apropiere"})
  		end
  	end)
  end
end, "Cere talonul unui jucator apropiat"}
end)
...
vRP.registerMenuBuilder({"police", function(add, data)
  ...
  if user_id ~= nil then
    ...
    if vRP.hasPermission({user_id,"police.askid"}) then
       choices["Verifica talonul"] = ch_talon
    end
